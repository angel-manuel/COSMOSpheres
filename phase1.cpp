...
bool phase1_moveto(float *dst) {
	...
	mathVecSubtract(delta, dst, &our_state[POS], DIMENSION);
	d = mathVecMagnitude(delta, DIMENSION);
	v = mathVecMagnitude(&our_state[VEL], DIMENSION);
	r = mathVecMagnitude(&our_state[RATE], DIMENSION);
